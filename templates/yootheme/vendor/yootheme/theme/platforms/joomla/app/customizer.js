jQuery(function ($) {
    ...
    var editor = $('.editor,.js-editor-tinymce'), wrapper = editor.length ? editor.parent() : article.parent();
    ...
    function init(active) {
        ...
        var editors = Joomla.editors.instances;

        $('.tm-editor').attr('hidden', !active);
        $('.editor,.js-editor-tinymce').attr('hidden', !!active);
        $(wrapper).toggleClass('tm-active', !!active);

        // fix codemirror
        if (editors.jform_articletext && editors.jform_articletext.scrollIntoView) {
